<template>
  <svg
    :width="size"
    :height="size"
    viewBox="0 0 48 48"
    xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    :style="svgStyle"
    :class="svgClass"
    v-bind="$attrs"
  >
    <g id="MessageFill-MessageFill" stroke="none" :stroke-width="1" fill="none" fill-rule="evenodd">
      <g id="MessageFill-编组">
        <rect
          id="MessageFill-矩形"
          fill="#FFFFFF"
          :opacity="0"
          :x="0"
          :y="0"
          :width="48"
          :height="48"
        />
        <path
          d="M24,5 C35.0454654,5 44,13.5065417 44,23.9999335 C44,34.3816935 35.234974,42.8187175 24.3517967,42.9970537 L24,42.999934 C20.5361038,43.0086689 17.1256565,42.1499296 14.0827434,40.5027329 L9.12272736,42.6506288 C8.20180797,43.0490167 7.13075643,42.6289802 6.73046083,41.7124519 C6.57362597,41.3533578 6.53805635,40.953324 6.6290919,40.5723965 L7.89727419,35.2705244 C5.44727043,32.1174388 4,28.2188348 4,23.999959 C4,13.5065671 12.9545346,5 24,5 Z M16.2500001,22 C15.2835003,22 14.5,22.7835045 14.5,23.75 C14.5,24.7164996 15.2835044,25.5 16.2499999,25.5 L16.2499999,25.5 C17.2164997,25.5 18,24.7164955 18,23.75 C18,22.7835004 17.2164956,22 16.2500001,22 L16.2500001,22 Z M32.2500001,22 C31.2835003,22 30.5,22.7835045 30.5,23.75 C30.5,24.7164996 31.2835044,25.5 32.2499999,25.5 L32.2499999,25.5 C33.2164997,25.5 34,24.7164955 34,23.75 C34,22.7835004 33.2164956,22 32.2500001,22 L32.2500001,22 Z M24.2500001,22 C25.2164956,22 26,22.7835004 26,23.75 C26,24.7164955 25.2164997,25.5 24.2500001,25.5 L24.2499999,25.5 C23.2835044,25.5 22.5,24.7164996 22.5,23.75 C22.5,22.7835045 23.2835003,22 24.2499999,22 L24.2500001,22 Z"
          id="MessageFill-形状结合"
          fill="currentColor"
          fill-rule="nonzero"
        />
      </g>
    </g>
  </svg>
</template>

<script setup lang="ts">
import { computed, useAttrs, type CSSProperties } from 'vue'

interface Props {
  size?: string | number
  color?: string
  className?: string
}

const props = withDefaults(defineProps<Props>(), {
  size: '1em',
  color: 'currentColor',
  className: '',
})

const attrs = useAttrs()

interface SvgStyle extends CSSProperties {
  verticalAlign: string | number
  color: string
}

const svgStyle = computed(
  (): SvgStyle => ({
    verticalAlign: '-0.125em',
    color: props.color,
    ...(attrs.style as CSSProperties),
  }),
)

const svgClass = computed((): string => {
  const classes: string[] = ['antd-mobile-icon']
  if (props.className) {
    classes.push(props.className)
  }
  if (attrs.class) {
    classes.push(attrs.class as string)
  }
  return classes.join(' ')
})

defineOptions({
  name: 'MessageFill',
  inheritAttrs: false,
})
</script>

<style scoped>
.antd-mobile-icon {
  display: inline-block;
  vertical-align: middle;
}
</style>
